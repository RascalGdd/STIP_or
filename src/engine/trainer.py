...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    print(f"\n>>> Epoch #{(epoch+1)}")
    for samples, targets, multiview_samples, points, video_samples, depths, points_video in metric_logger.log_every(data_loader, print_freq, header):
        ...
        outputs = model(samples, targets, multiview_samples, points, video_samples, depths, points_video)
