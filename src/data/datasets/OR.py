...
class MultiView_CocoDetection(VisionDataset):
    ...
    def _load_tiff(self, id: int):
        path = self.coco.loadImgs(id)[0]["file_name"]
        depth_path = os.path.join(self.root.replace("images", "depth"), path.replace("jpg", "tiff"))
        cv2_img = cv2.imread(depth_path)
        depth_data = torch.tensor(cv2_img).type(torch.FloatTensor).permute(2, 0, 1)
        return depth_data
...
def make_coco_transforms(image_set):
    ...
    scales = [480, 512, 544, 576, 608, 640, 672, 704, 736, 768, 800, 832, 864, 896, 928, 960]

    if image_set == 'train':
        return T.Compose([
            T.RandomHorizontalFlip(),
            T.ColorJitter(.4, .4, .4),
            T.RandomResize(scales, max_size=2048),
            normalize])

    if image_set == 'val' or "test" or "infer":
        return T.Compose([
            T.RandomResize([960], max_size=2048),
            normalize,
        ])
